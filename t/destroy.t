...
my %router;
# routers with a single branch used to be freed by perl itself during global
# destruction, before DESTROY had a chance to run
our @single;
...
push @single, Router::R3->new( "/api/v1/single/$_/{id}" => sub {} ) for 1 .. 50;
ok( $single[0]->match( '/api/v1/single/1/42' ) );
